            ...
            if (HAS_OWM) {
                weatherResult = await Promise.all(
                  INDIAN_CITIES.map(async (city) => {
                    try {
                      const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${city.lat}&lon=${city.lon}&appid=${OWM_KEY}&units=metric`);
                      ...
                      if (res.ok) {
                        ...
                        return {
                            ...city, condition: 'Unknown', description: 'Data unavailable', icon: '01d',
                            temp_c: 0, wind_kph: 0, humidity: 0, is_storm: false,
                            summary: 'Data unavailable', wind_deg: 0
                        } as CityWeather;
                    }
                  })
                );
            ...
            <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
                ...
                <div style={{ flex: 1, position: 'relative', minWidth: 0 }}>
                    ...
                    <MapContainer center={mapCenter} zoom={5} style={{ height: '100%', width: '100%' }} zoomControl>
                        ...
                        {cities.filter(c => c.condition !== 'Unknown').map(c => {
...
print("WeatherMap updated successfully.")
