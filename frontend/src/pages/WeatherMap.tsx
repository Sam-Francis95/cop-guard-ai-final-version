...
const STORM_CONDITIONS = ['thunderstorm', 'tornado', 'squall', 'drizzle', 'heavy intensity rain', 'very heavy rain', 'extreme rain', 'heavy snow', 'heavy shower rain'];
...
const getWeatherIcon = (condition: string) => {
    ...
    if (c.includes("mist") || c.includes("haze") || c.includes("fog") || c.includes("smoke") || c.includes("dust"))
        return "🌥️";
...
function MapBoundsController({ onBoundsChange }: { onBoundsChange: (bounds: L.LatLngBounds, zoom: number) => void }) {
    const map = useMap();
...
export default function WeatherMap() {
    ...
    const [gridWeather, setGridWeather] = useState<CityWeather[]>([]);
    const fetchedGridRef = useRef<{ lat: number, lon: number }[]>([]);

    const handleBoundsChange = useCallback(async (bounds: L.LatLngBounds, zoom: number) => {
        if (!HAS_OWM) return;

        let gridSize;
        if (zoom <= 5) gridSize = 2;
        else if (zoom <= 7) gridSize = 4;
        else if (zoom <= 10) gridSize = 6;
        else gridSize = 8;
        ...
        for (let i = 0; i <= gridSize; i++) {
            for (let j = 0; j <= gridSize; j++) {
                ...
                for (const c of INDIAN_CITIES) {
                    if (Math.sqrt(Math.pow(c.lat - lat, 2) + Math.pow(c.lon - lon, 2)) < 0.6) {
                        isNearby = true;
                        break;
                    }
                }
                ...
                for (const g of fetchedGridRef.current) {
                    if (Math.sqrt(Math.pow(g.lat - lat, 2) + Math.pow(g.lon - lon, 2)) < 0.6) {
                        isNearby = true;
                        break;
                    }
                }
                ...
                if (!isNearby) {
                    ...
                    fetchedGridRef.current.push({ lat, lon });
                }
            }
        }

        const pointsToFetch = newPoints.slice(0, 20);
        if (pointsToFetch.length === 0) return;

        console.log(`📡 Fetching ${pointsToFetch.length} dynamic grid zones at zoom ${zoom}...`);

        const results: CityWeather[] = [];

        await Promise.all(pointsToFetch.map(async (pt, idx) => {
            try {
                await new Promise(resolve => setTimeout(resolve, idx * 300));
                const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${pt.lat}&lon=${pt.lon}&appid=${OWM_KEY}&units=metric`);
                if (!res.ok) return;
                const data = await res.json();

                if (data.cod !== 200 || !data.main || !data.weather) return;

                const condition = data.weather[0].description;
                const is_storm = STORM_CONDITIONS.some(s => condition.toLowerCase().includes(s));

                results.push({
                    name: data.name || `Local Zone`,
                    lat: pt.lat,
                    lon: pt.lon,
                    condition: condition,
                    ...
                    is_storm,
                    summary: `${condition}, ${Math.round(data.main.temp)}°C`,
                    wind_deg: data.wind.deg || getWindDeg(pt.lat, pt.lon)
                } as CityWeather);
            } catch (e) { }
        }));
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            if (token) {
                try {
                    const [wR, clR] = await Promise.all([
                        fetch(`${import.meta.env.VITE_API_URL}/api/workers/locations`, { headers }),
                        fetch(`${import.meta.env.VITE_API_URL}/api/claims`, { headers }),
                    ]);
                    if (!wR.ok || !clR.ok) throw new Error("API failed");
                    wd = await wR.json();
                    cld = await clR.json();
                } catch (err) {
                    console.error("API failed → fallback mode", err);
                }
            }

            let weatherResult: CityWeather[] = [];

            if (HAS_OWM) {
                weatherResult = await Promise.all(
                    INDIAN_CITIES.map(async (city, index) => {
                        try {
                            // Small delay to prevent bursting API limit
                            await new Promise(resolve => setTimeout(resolve, index * 300));
                            const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city.name}&appid=${OWM_KEY}&units=metric`);

                            if (!res.ok) {
                                console.error("FAILED CITY:", city.name, "Response not ok: HTTP", res.status);
                            ...
                            if (data.cod !== 200 || !data.main || !data.weather) {
                                console.error("FAILED CITY:", city.name, data);
                                throw new Error("Invalid format or missing fields");
                            }

                            const condition = data.weather[0].description;
                            const desc = data.weather[0].description;
                            const is_storm = STORM_CONDITIONS.some(s => condition.toLowerCase().includes(s) || desc.toLowerCase().includes(s));
                            ...
                            return {
                                ...
                                humidity: data.main.humidity,
                                is_storm,
                                summary: `${desc}, ${Math.round(data.main.temp)}°C`,
                                wind_deg: data.wind.deg || getWindDeg(city.lat, city.lon)
                            } as CityWeather;
                        } catch (err: any) {
                            console.error("Fetch failed:", city.name, err);
                            return {
                                ...city, condition: 'Unavailable', description: 'Unavailable', icon: '🌍',
                                temp_c: '--', wind_kph: '--', humidity: '--', is_storm: false,
                                summary: 'Unavailable', wind_deg: 0, error: true
                            } as any;
                        }
                    })
                );
            } else {
                weatherResult = INDIAN_CITIES.map(city => ({
                    ...city, condition: 'Unavailable', description: 'Unavailable', icon: '🌍',
                    temp_c: '--', wind_kph: '--', humidity: '--', is_storm: false,
                    summary: 'Unavailable', wind_deg: 0, error: true
                } as any));
            ...
            console.log("FINAL CITY DATA:", weatherResult);

            setCities(weatherResult);
    ...
    useEffect(() => {
        fetchAll();
        const t = setInterval(() => fetchAll(), 180000);
            ...
            <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
                ...
                <div style={{ flex: 1, position: 'relative', minWidth: 0 }}>
                    ...
                    <MapContainer center={mapCenter} zoom={5} style={{ height: '100%', width: '100%' }} zoomControl>
                        ...
                        <MapBoundsController onBoundsChange={handleBoundsChange} />

                        {/* City Weather Icons & Local Geo-Zones (Tasks 1, 4, 5, 6) */}
                        {[...cities.filter(c => c.condition !== 'Unknown'), ...gridWeather].map((c, idx) => {
                ...
                <div style={{ width: 300, borderLeft: '1px solid #1f2937', background: '#0d1117', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                        {cities.map(city => {
                            if (city.condition === 'Unknown') {
                                return (
                                    <div key={city.name} style={{ padding: '14px 18px', borderBottom: '1px solid #1f2937', borderLeft: `3px solid #1f2937` }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            <div>
                                                <p style={{ fontWeight: 700, fontSize: 14, color: '#f9fafb' }}>{city.name}</p>
                                                <p style={{ fontSize: 11, color: '#ef4444', marginTop: 2 }}>{city.description}</p>
                                            </div>
                                        </div>
                                    </div>
                                );
